...
class GTO_Atten(nn.Module): 
    def __init__(self, 
        ...
        # atten_1
        self.kv_1 = nn.Linear(c_dim, c_dim) # 生成 key / value
        ...
        # Xavier
        nn.init.xavier_uniform_(self.Q) # 均匀初始化，提高稳定性
    ...
    def forward(self, W0):   
        ...
        kv_1 = rearrange(self.kv_1(W0), 'b n (h c) -> b h n c', h=self.n_heads) # 线性变换重排
        ...
        q_3 = rearrange(self.q_3(W0), 'b n (h c) -> b h n c', h=self.n_heads) # 将原输入映射为 query
        re_proj_weights = F.softmax(q_3 @ k.transpose(-2,-1)/ q_3.shape[-1]**0.5, dim=-1)
        
        W = torch.einsum('bhnm, bhmc -> bhnc', re_proj_weights, v)
        W = rearrange(W, 'b h n c -> b n (h c)') # 是 reshape 的加强版本
