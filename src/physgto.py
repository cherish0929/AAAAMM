...
class GatedGNN(nn.Module):
    ...
    asymmetric velocity gradients (e.g., strong gradient on one side of melt pool boundary).
    """
...
class Decoder(nn.Module):
    ...
    separate Ux/Uy/Uz heads. Each head has its own deeper architecture
    because the three velocity components have very different physics
    (Uz dominated by buoyancy, Ux/Uy by Marangoni surface tension gradients).
    """
    def __init__(self,
                 N = 4,
                 enc_dim=128,
        ...
        # Uses Tanh output so scale is in [-1, 1], making effective multiplier in [0, 2]
    ...
    def forward(self, V_all, pos_enc):
        ...
        # 2-layer gated residual (deeper backbone for nonlinear velocity)
...
class Encoder(nn.Module):
    ...
    Instead of simple additive fusion of time/condition, uses multiplicative
    modulation which is much better at capturing how time/conditions
    *modulate* the velocity field rather than just shifting it.
    """
    def __init__(self,
                 space_size=2,
                 state_size=4,
        ...
        # FiLM conditioning: produces (gamma, beta) for multiplicative + additive modulation
        # This captures how time/conditions *modulate* velocity dynamics,
        # not just shift it (which additive fusion does)
        self.film_time = nn.Sequential(
            nn.Linear(enc_t_dim, enc_dim),
            nn.SiLU(),
            nn.Linear(enc_dim, enc_dim * 2),  # gamma and beta
        )
    ...
    def forward(self, node_pos, state_in, time_i, conditions, edges):
        ...
        # Compute velocity magnitude and per-component sign as auxiliary features
        vel_mag = torch.norm(state_in, dim=-1, keepdim=True)  # [bs, N, 1]
        vel_sign = torch.sign(state_in)  # [bs, N, state_size] — captures flow direction

        # node embedding: state + position + velocity magnitude + signs
        state_aug = torch.cat((state_in, node_pos, vel_mag, vel_sign), dim=-1)
        V = self.fv1(state_aug)

        # FiLM conditioning: gamma * V + beta (much more expressive than additive)
        time_film = self.film_time(time_i)  # (bs, enc_dim*2)
        t_gamma, t_beta = time_film.chunk(2, dim=-1)
        V = V * (1.0 + t_gamma.unsqueeze(-2)) + t_beta.unsqueeze(-2)

        cond_film = self.film_cond(conditions)  # (bs, enc_dim*2)
        c_gamma, c_beta = cond_film.chunk(2, dim=-1)
        V = V * (1.0 + c_gamma.unsqueeze(-2)) + c_beta.unsqueeze(-2)

        # Edge embedding: spatial info + velocity difference + magnitude difference
        ...
        # Velocity magnitude difference along edges (captures gradient strength)
        send_mag = torch.norm(send_vel, dim=-1, keepdim=True)
        recv_mag = torch.norm(recv_vel, dim=-1, keepdim=True)
        mag_diff = recv_mag - send_mag  # [bs, ne, 1]
...
class Model(nn.Module):
    ...
    def autoregressive(self,
        ...
        for t in range(T):
            ...
            # so high-velocity regions get more noise (they're more robust to it)
            # and near-zero regions get almost no noise (preserving the zero structure)
            if self.training and t > 0:
                vel_mag = state_t.abs().clamp_min(1e-3)
                noise_scale = 0.005  # reduced from 0.02
